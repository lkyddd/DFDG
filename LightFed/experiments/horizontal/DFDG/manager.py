...
class ServerManager(BaseServer):
    def __init__(self, ct, args):
        ...
        self.generator_2 = generator_model_pull(self.super_params).to(self.device)
        ...
        self.beta_cd = args.beta_cd
        self.beta_ey = args.beta_ey

        self.gamma = args.gamma

        self.condition_ = args.condition_
        self.dual_noise = args.dual_noise
    ...
    def fed_finish_client_train_step(self,
        ...
        if not self.unfinished_client_num:
            ...
            for i in range(self.I):
                ...
                if self.dual_noise == True:
                    self.z2 = Variable(self.FloatTensor(np.random.normal(0, 1, (self.batch_size, self.latent_dim))))
                    self.y2 = self.random_choice_y(self.batch_size, self.label_num)
                else:
                    self.z2 = self.z1
                    self.y2 = self.y1
                ...
                self.Update_generator(self.generator_1, self.generator_2, self.z1, self.y1, self.z2, self.y2, gen_style='1')
                self.Update_generator(self.generator_2, self.generator_1, self.z2, self.y2, self.z1, self.y1, gen_style='2')
                ...
                if self.save_generator == True:
                    if i in [0, int(self.I * 0.2)-1, int(self.I * 0.4)-1, int(self.I * 0.6)-1, int(self.I * 0.8)-1, int(self.I * 1.0)-1]:
                        torch.save((self.generator_1, self.generator_2), \
                        f"{os.path.dirname(__file__)}/generator/{self.data_set}_{self.noise_label_combine}_{str(i)}_{self.super_params['non_iid_alpha']}_{self.seed}.pth")
    ...
    def Update_generator(self, trian_generator, generator, update_z, update_y, z, y, gen_style='1'):
        ##update generator_1
        ...
        self.model.eval()
        LOSS_GEN = 0
        L_FID, L_TRAN, L_DIV, L_EY, L_CD = 0, 0, 0, 0, 0
        ##optimizer
        ...
        for _ in range(self.I_g):
            ...
            ## Obtain output of the other generator
            gen_labels_o = Variable(self.LongTensor(np.array(y)))

            # Generate a batch of images
            gen_imgs_o, _, _ = generator(z, gen_labels_o)

            L_kl_logit_o = self.cal_client_logits(gen_imgs_o, y)
            clients_logit_o_softmax = F.softmax(L_kl_logit_o / self.temp, dim=1).clone().detach()

            ## obtain index
            update_index, index = [], []
            for u_ind_, u_y_ in enumerate(update_y):
                for ind_, y_ in enumerate(y):
                    if self.condition_ == True:
                        if u_y_ == y_:
                            update_index.append(u_ind_)
                            index.append(ind_)
                    else:
                        update_index.append(u_ind_)
                        index.append(ind_)

            L_cd = - self.KL_batchmean(clients_logit_log_softmax[update_index], clients_logit_o_softmax[index])

                        
            if self.noise_label_combine in ['mul', 'add', 'cat']:
                loss_gen =  L_fid + self.beta_tran * L_tran + self.beta_div * L_div + self.beta_cd * L_cd # + self.beta_ey * L_ey
            else:
                loss_gen =  L_fid + self.beta_tran * L_tran + self.beta_div * L_div + self.beta_cd * L_cd
            loss_gen.backward()
            optimizer_G.step()
        ...
        # print(info_gen)
        if gen_style == '1':
            info_gen = {'comm. round': self.step, 'client_id': 'server', 'update': 'generator1',
                    ...
                    'L_CD': round(L_CD / self.I_g, 4)}
            self.server_train_test_res.update(generator_update1=info_gen)
        else:
            info_gen = {'comm. round': self.step, 'client_id': 'server', 'update': 'generator2',
                    'LOSS_GEN': round(LOSS_GEN / self.I_g, 4), 'L_FID': round(L_FID / self.I_g, 4),
                    'L_TRAN': round(L_TRAN / self.I_g, 4), 'L_DIV': round(L_DIV / self.I_g, 4), 'L_EY': round(L_EY / self.I_g, 4),
                    'L_CD': round(L_CD / self.I_g, 4)}
            self.server_train_test_res.update(generator_update2=info_gen)
    
    def Update_global_model(self,):
        grad_False(self.generator_1)
        self.generator_1.eval()
        grad_False(self.generator_2)
        ...
        for _ in range(self.I_d):
            ...
            # Sample noise as generator input
            L_kl_1 = self.cal_L_kl(self.generator_1, self.z1, self.y1)
            L_kl_2 = self.cal_L_kl(self.generator_2, self.z2, self.y2)

            loss_glo = L_kl_1 + L_kl_2

            loss_glo.backward()
            optimizer_model.step()
            LOSS_GLO += loss_glo.detach().cpu().numpy()
            L_KL_1 += L_kl_1.detach().cpu().numpy()
            L_KL_2 += L_kl_2.detach().cpu().numpy()
