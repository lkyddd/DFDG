...
class ServerManager(BaseServer):
    ...
    def fed_finish_client_train_step(self,
        ...
        if not self.unfinished_client_num:
            ...
            for i in range(self.I):
                ...
                self.y = self.random_choice_y(self.batch_size, self.label_num)
                ##
                ...
                if self.save_generator == True:
                    if i in [0, int(self.I * 0.2)-1, int(self.I * 0.4)-1, int(self.I * 0.6)-1, int(self.I * 0.8)-1, int(self.I * 1.0)-1]:
                        ...
                        f"{os.path.dirname(__file__)}/generator/{self.data_set}_{str(i)}_{self.seed}.pth")
    ...
    def Update_generator(self, z, y):
        ...
        for _ in range(self.I_g):
            ...
            loss_gen =  L_cls - self.beta_md * L_md + self.beta_div * L_div #+ self.beta_ey * L_ey
            loss_gen.backward()
        ...
        info_gen = {'comm. round': self.step, 'client_id': 'server', 'update': 'generator',
                    'LOSS_GEN': round(LOSS_GEN / self.I_g, 4), 'L_CLS': round(L_CLS / self.I_g, 4),
                    'L_MD': round(L_MD / self.I_g, 4), 'L_DIV': round(L_DIV / self.I_g, 4)} # , 'L_EY': round(L_EY / self.I_g, 4)
        # print(info_gen)
    ...
    def Update_global_model(self, z, y):
        ...
        for _ in range(self.I_d):
            ...
            loss_glo = self.cal_L_kl(self.generator, z, y)
        ...
        info_global = {'comm. round': self.step, 'client_id': 'server', 'update': 'global_model',
                'LOSS_GLO': round(LOSS_GLO / self.I_d, 4)}
        # print(info_global)
